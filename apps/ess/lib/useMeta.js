...
// useMeta — Program P1.7. One fetch of GET /api/hr/me/meta (customer session)
// serves the enum vocabularies the ESS app used to hardcode: genders, marital
// statuses, dependant relations, education levels, address types, etc.
//
...
// "CIVIL_UNION" → "Civil Union" — the shared SNAKE → Title-case humanizer.
...
function fetchMeta() {
  ...
  if (!inflight) {
    inflight = apiGet('/api/hr/me/meta')
      .then((body) => {
