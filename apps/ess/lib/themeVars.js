// Derive the `--theme-*` CSS-variable map from a theme resolved by
// @hr/theme-engine's resolveTenantTheme(). This mirrors the variable contract
// the platform app applies (lib/themes.js getThemeVars): same keys, same
// derivations (on-color contrast pick, darken, muted/border alpha) so branded
// components read identical variables across apps.
//
...
// fixed style. We compute the rest here.
...
export function themeVarsFromResolved(theme) {
  ...
  const text = isHex(palette.foreground) ? palette.foreground : '#0F172A';
  return {
