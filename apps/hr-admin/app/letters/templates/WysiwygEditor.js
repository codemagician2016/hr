...
// Must match backend letters.service.js DEFAULT_STAMP_BOX.
...
const WysiwygEditor = forwardRef(function WysiwygEditor(
  {
    value, onChange, letterhead,
    signatureUrl, signatureBox, onSignatureBoxChange,
    stampUrl, stampBox, onStampBoxChange,
  }, ref,
) {
  ...
  // Drag/resize is target-aware: the gesture stores which overlay (signature or
  // stamp) it belongs to plus that target's apply fn, so one handler serves both.
  ...
  const onSigDown = useCallback((e, gestureMode, target) => {
    const apply = target === 'stamp' ? onStampBoxChange : onSignatureBoxChange;
    const startBox = target === 'stamp' ? (stampBox || DEFAULT_STAMP_BOX) : (signatureBox || DEFAULT_SIG_BOX);
    if (!apply || !stageRef.current) return;
    e.preventDefault(); e.stopPropagation();
    const r = stageRef.current.getBoundingClientRect();
    sigDragRef.current = {
      mode: gestureMode, startX: e.clientX, startY: e.clientY,
      w: r.width, h: r.height, start: { ...startBox }, apply,
    };
    window.addEventListener('pointermove', onSigMove);
    window.addEventListener('pointerup', onSigUp);
  }, [onSignatureBoxChange, onStampBoxChange, signatureBox, stampBox, onSigMove, onSigUp]);
  useEffect(() => () => {
    window.removeEventListener('pointermove', onSigMove);
    window.removeEventListener('pointerup', onSigUp);
  }, [onSigMove, onSigUp]);
        ...
        <div className="border border-gray-200 rounded-lg bg-gray-50 p-3 overflow-auto" style={{ maxHeight: '58vh' }}>
          <div ref={stageRef} className="relative inline-block mx-auto">
            ...
            <div
              ...
              style={editorStyle}
            />
            {/* draggable static signature + stamp (each independently placeable) */}
            {canvas.cssWidth ? [
              { key: 'signature', url: signatureUrl, box: signatureBox || DEFAULT_SIG_BOX, colour: '#ec4899', label: 'Signature' },
              { key: 'stamp', url: stampUrl, box: stampBox || DEFAULT_STAMP_BOX, colour: '#0ea5e9', label: 'Stamp' },
            ].filter((o) => o.url).map((o) => (
              <div
                key={o.key}
                className="absolute"
                style={{
                  left: o.box.x * canvas.cssWidth, top: o.box.y * canvas.cssHeight,
                  width: o.box.w * canvas.cssWidth, height: o.box.h * canvas.cssHeight,
                  border: `1px dashed ${o.colour}`, cursor: 'move', touchAction: 'none',
                }}
                onPointerDown={(e) => onSigDown(e, 'move', o.key)}
                title={`${o.label} — drag to move · drag the corner to resize`}
              >
                <img src={o.url} alt={o.label} className="w-full h-full object-contain pointer-events-none select-none" />
                <span
                  onPointerDown={(e) => onSigDown(e, 'resize', o.key)}
                  className="absolute rounded-sm"
                  style={{ right: -5, bottom: -5, width: 10, height: 10, background: o.colour, cursor: 'nwse-resize' }}
                />
              </div>
            )) : null}
          </div>
